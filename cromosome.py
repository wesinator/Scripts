...
__author__ = "Fernando Ramírez & Antonio Sánchez {framirez, asanchez}@koodous.com"
...
help_description = """
    ...
    All given APK/Zip files will be extracted and the strings will be retrieved
    from them

    {framirez,asanchez}@koodous.com | 2015
...
def get_zip_files_strings(zip_file, files_include, min_string_length):
    print '\x1b[%sm%s\x1b[0m' % (';'.join(['32']), zip_file), " UnZipping file and getting the strings"
...
def main():
    ...
    parser.add_argument('-a', nargs='+', action='append', dest='a_group', default=[], \
        help='Add files to compare A group')
    parser.add_argument('-b', nargs='+', action='append', dest='b_group', default=[], \
    ...
    #Statistics
    if results.a_group:
        print '\x1b[%sm%s\x1b[0m' % (';'.join(['1']), str(len(a_group))), "\tcommon string found between all files in group A"

    if results.b_group:
        print '\x1b[%sm%s\x1b[0m' % (';'.join(['1']), str(len(b_group))), "\tunion strings found in group B"

    if results.a_group and results.b_group:
        print '\x1b[%sm%s\x1b[0m' % (';'.join(['1']), str(len(a_group - b_group))), "\tstring from A group not present in group B"
